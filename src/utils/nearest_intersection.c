...
t_intersection	*ft_get_nearest_intersection(t_ray *ray, t_scene *scene)
{
	...
	while (current)
	{
		...
		if (sphere->enable_intersection && (dist = get_sphere_distance(ray,
					sphere)) < nearest.distance)
		{
	...
	while (current)
	{
		...
		if (cylinder->enable_intersection && (dist = get_cylinder_distance(ray,
					cylinder)) < nearest.distance)
		{
	...
	while (current)
	{
		...
		if (plane->enable_intersection && (dist = get_plane_distance(ray,
					plane)) < nearest.distance)
		{
	...
	while (current)
	{
		...
		if (cone->enable_intersection && (dist = get_cone_distance(ray,
					cone)) < nearest.distance)
		{
			nearest.object = cone;
			nearest.distance = dist;
			nearest.type = CONE;
